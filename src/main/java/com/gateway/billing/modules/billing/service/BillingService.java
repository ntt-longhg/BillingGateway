package com.gateway.billing.modules.billing.service;
...
import com.gateway.billing.modules.billing.dto.BillingWebhookRequest;
...
import com.gateway.billing.modules.servicecatalog.model.Service;
import com.gateway.billing.modules.servicecatalog.model.ServicePrice;
import com.gateway.billing.modules.servicecatalog.repository.PriceTierRepository;
import com.gateway.billing.modules.servicecatalog.repository.ServicePriceRepository;
import com.gateway.billing.modules.servicecatalog.repository.ServiceRepository;
...
import com.gateway.billing.modules.usagelog.model.FeeBreakdownStructure;
...
public class BillingService {
    ...
    public BillingWebhookResponse processBilling(Tenant tenant, BillingWebhookRequest request) {
        ...
        Service service = serviceRepository.findByCode(request.getServiceCode())
                .orElseThrow(() -> {
                    ...
                    return new ResourceNotFoundException("Service", "code", request.getServiceCode());
                });
        log.info("Service found: {} (id={})", service.getName(), service.getId());

        // 2. Find active service price
        List<ServicePrice> activePrices = servicePriceRepository.findByServiceIdAndIsActiveTrue(service.getId());
        if (activePrices.isEmpty()) {
            log.error("No active price found for service: {}", service.getCode());
            throw new BusinessException("NO_ACTIVE_PRICE", "No active price found for service: " + service.getCode());
        }
        ...
        log.info("Wallet locked (SELECT FOR UPDATE): id={} balance={} creditLimit={} available={}",
                wallet.getId(), wallet.getBalance(), wallet.getCreditLimit(), wallet.getAvailableBalance());
        ...
        if (wallet.getAvailableBalance().compareTo(totalFee) < 0) {
            log.error("Insufficient balance: available={} required={}", wallet.getAvailableBalance(), totalFee);
            throw new BusinessException("INSUFFICIENT_BALANCE",
                    "Insufficient balance. Available: " + wallet.getAvailableBalance() + ", Required: " + totalFee);
        }
        ...
        Transaction transaction = Transaction.builder()
                ...
                .description(request.getDescription() != null ? request.getDescription() : "Charge for " + service.getCode())
        ...
        // 9. Create UsageLog
        FeeBreakdownStructure feeBreakdownStructure = buildFeeBreakdown(servicePrice, request.getUsageUnits(), totalFee);
        ...
        // 10. Build response
        BillingWebhookResponse.FeeBreakdownDto feeBreakdownDto = BillingWebhookResponse.FeeBreakdownDto.builder()
                ...
                .subsequentUnits(request.getUsageUnits() > servicePrice.getInitialSize()
                        ? request.getUsageUnits() - servicePrice.getInitialSize() : 0)
                .totalFee(totalFee)
                .build();
     ...
     * First N units charged at initialFee, remaining units charged at subsequentFee.
     */
    ...
    private FeeBreakdownStructure buildFeeBreakdown(ServicePrice servicePrice, int usageUnits, BigDecimal totalFee) {
        ...
        if (usageUnits <= servicePrice.getInitialSize()) {
            ...
            int subsequentTiers = (int) Math.ceil((double) remainingUnits / servicePrice.getSubsequentSize());
            breakdown.setSubsequentFeeApplied(servicePrice.getSubsequentFee().multiply(BigDecimal.valueOf(subsequentTiers)));
        }
